...
/// <summary>
/// 트레일러(플레이 영상) 촬영용 자유 시점 카메라 — 에디터·개발 빌드 전용. GameScene에 자동 생성된다.
///
///  · 관전자(닉네임 !관전!, <see cref="TrailerMode"/>)로 들어왔으면 자동으로 켜진다. 그 외엔 F9로 토글(개발자 촬영용).
///  · 우선순위 100 CinemachineCamera라 켜면 Brain이 플레이어 카메라에서 부드럽게 넘어오고, 끄면 되돌아간다.
///  · 켜져 있는 동안 내 캐릭터 입력은 GameplayInputBlocker.TrailerBlocked로 막힌다(WASD는 카메라가 쓴다).
///
/// 조작
///  이동  W/A/S/D · 위/아래 E/Q(또는 Space/Ctrl) · Shift 3배 · Alt 0.3배 · 휠 = 기본 속도
///  시선  우클릭 드래그
///  1/2/3 프리셋 — 탑다운(오버쿡드) / 쿼터뷰 / 로우앵글. 그리드 중앙(팔로우 중이면 대상)을 바라본다
///  Tab   팔로우 대상 순환(플레이어)   F  팔로우 켬/끔(현재 오프셋 유지, 둥실 따라감)
...
public sealed class TrailerCamera : MonoBehaviour
{
    ...
    private static void OnSceneLoaded(Scene scene, LoadSceneMode _)
    {
        ...
        var cam = new GameObject("~TrailerCamera").AddComponent<TrailerCamera>();   // 씬 오브젝트 — 씬이 바뀌면 함께 정리
        cam.m_AutoEnable = TrailerMode.LocalIsSpectator;
    }
    ...
    private void Update()
    {
        ...
        if (m_AutoEnable && !m_Active && Camera.main != null) { m_AutoEnable = false; Enable(); }
        if (!m_Active) return;
        ...
        {
            TrailerMode.HideNametags = !TrailerMode.HideNametags;
            Debug.Log($"[TrailerCam] 이름표 {(TrailerMode.HideNametags ? "숨김" : "표시")}");
        }
    }
    ...
    private void Enable()
    {
        ...
        GameplayInputBlocker.TrailerBlocked = true;
        SetHudHidden(true);
    ...
    private void Disable()
    {
        ...
        SetHudHidden(false);
        TrailerMode.HideNametags = false;
        // 관전자는 몸이 없으니 입력을 계속 막아둔다(PlayerUnit이 켠 상태 유지)
        if (!TrailerMode.LocalIsSpectator) GameplayInputBlocker.TrailerBlocked = false;
    ...
    {
        if (m_Active) Disable();
        if (TrailerMode.LocalIsSpectator) GameplayInputBlocker.TrailerBlocked = false;   // 씬을 떠나면 잠금 해제
    }
    ...
    private void CycleFollowTarget()
    {
        ...
        foreach (var p in FindObjectsByType<Player.PlayerUnit>(FindObjectsSortMode.InstanceID))
            if (p != null && !p.IsSpectator) m_Players.Add(p);
