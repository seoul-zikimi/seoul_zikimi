...
public static class GameplayInputBlocker
{
    ...
    {
        get => s_Manual || MatchGateBlocked || TrailerBlocked || DialogueBlocked;
        set => s_Manual = value;
    }
    /// <summary>촬영 카메라(TrailerCamera)가 켜진 동안 — 내 캐릭터가 WASD·클릭에 반응하지 않고 제자리에 선다.</summary>
    public static bool TrailerBlocked { get; set; }
    /// <summary>튜토리얼 대화를 다 읽기 전(퀘스트 안내 줄이 뜨기 전)까지 — 읽지 않고 움직이는 걸 막는다(TutorialDialogueHUD가 관리).</summary>
    public static bool DialogueBlocked { get; set; }
