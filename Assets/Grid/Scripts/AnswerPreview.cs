    ...
    public class AnswerPreview : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (ghost)
            {
                ...
                float ha = 0.36f + 0.14f * Mathf.Abs(Mathf.Sin(Time.time * 5f));     // 강조: 밝고 빠른 펄스(0.45~0.60은 실물 블록처럼 보여 낮춤)
        ...
        private const float kGhostAlphaBase = 0.13f;                                           // 고스트 기본 알파(맵 미설정 시). 0.16은 "이미 지어진 것처럼 보인다"는 피드백 → 0.10(안 보임) → 0.13
