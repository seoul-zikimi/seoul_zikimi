    ...
    public sealed class Weather3DVfxRig : MonoBehaviour
    {
        ...
        public void CoverArea(Bounds area, float height = 9f)
        {
            ...
            foreach (var ps in m_All)
            {
                ...
                var em = ps.emission; em.rateOverTime = b.rate * mul;
                var main = ps.main; main.maxParticles = Mathf.CeilToInt(b.max * mul);
                ...
                if (vy > 0.05f)
                {
                    ...
                    if (main.startLifetime.constant < need) { main.startLifetime = need; main.maxParticles = Mathf.CeilToInt(b.rate * mul * need * 1.35f); }
                }
            }
        }
