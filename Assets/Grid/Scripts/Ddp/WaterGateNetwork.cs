...
namespace GridSystem
{
    ...
    public class WaterGateNetwork : DdpGimmickBase
    {
        ...
        // 1순위는 Toon Water(광통교·롯데 호수와 같은 셰이더 — Resources/Materials/Mat_DdpFlowWater):
        // 거품·물결이 있는 카툰 물. 색·거품은 머티리얼 에셋에 구워져 있어 런타임 틴트는 생략한다.
        private static Material s_Mat;
        private static bool s_MatIsToon;

        private static void EnsureWaterMaterial()
        {
            if (s_Mat != null) return;
            var toon = Resources.Load<Material>("Materials/Mat_DdpFlowWater");
            if (toon != null)
            {
                s_Mat = new Material(toon) { hideFlags = HideFlags.HideAndDontSave };   // 사본 — 에셋 오염 방지
                s_MatIsToon = true;
                return;
            }
        ...
        {
            EnsureWaterMaterial();
            if (s_Mat == null || s_MatIsToon) return;   // 툰 워터는 색·거품이 에셋에 구워져 있음 — Lit 폴백만 틴트
