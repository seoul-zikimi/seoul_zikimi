...
namespace GridSystem
{
    ...
    /// 기믹 ② 유구 발굴터   : 물이 빠진 동안에만 노출. 올라서서 버티면 보너스 재료가 출토된다.
    ///                        물이 차면 잠긴다 — ①과 한 사이클로 맞물린다.
    ...
    public class DdpGimmickConfig : ScriptableObject
    {
        ...
        [Header("② 유구 발굴터")]
        [Tooltip("발굴터 위에서 버텨야 하는 시간(초)")] public float DigSeconds = 2.5f;
        [Tooltip("발굴터 반경(m) — 이 안에 서 있어야 게이지가 찬다")] public float DigRadius = 1.8f;
        [Tooltip("출토 후 다음 유구가 드러날 때까지(초). 물이 한 번 차고 빠지면 즉시 새로 드러난다")]
